...
parser.add_argument("-f", "--file", help="Path to db.yaml file")

# 添加参数，用于指定MySQL的主机名
parser.add_argument("-H", "--host", help="MySQL host")

# 添加参数，用于指定MySQL的端口号
parser.add_argument("-P", "--port", default=3306, type=int, help="MySQL port(default: 3306)")
...
# 添加版本号参数
parser.add_argument('-v', '--version', action='version', version='sqlai_helper工具版本号: 2.1.3，更新日期：2024-10-10 <-> 支持SQL改写，合并LLM模型接口')
...
if args.file:
    # 如果使用了-f参数，则必须只用-f参数且不能再使用其他MySQL配置参数
    if args.host or args.user or args.password or args.database:
        parser.error("-f/--file参数与其他MySQL配置参数不能共存。")
    ...
    # 如果没有使用-f参数，则必须指定所有MySQL配置参数
    if not all([args.host, args.user, args.password, args.database]):
        parser.error("必须指定所有MySQL配置参数，包括-H/--host、-P/--port、-u/--user、-p/--password和-d/--database。")
        ...
        if has_table_alias(table_aliases) is False and contains_dot is False:
            ...
            if group_by_fields is not None and len(group_by_fields) != 0:
                for group_field in group_by_fields:
                    #print(f"调试-表名：{table_name}, 分组字段:{group_field}")
...
print("\n")
print("\033[1m5) AI的建议：\033[0m")
print("-" * 100)
optimized_sql = optimize_sql(formatted_sql)
