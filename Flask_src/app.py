...
def analyze_sql(sql_query, db_config, sample_size=100000):
    ...
    try:
        ...
        if 'SELECT' not in sql_query.upper():
            return {"error": "sql_helper工具仅支持select语句"}
    except Exception as e:
        return {"error": f"解析 SQL 出现语法错误：{str(e)}"}
    ...
    try:
        ...
        return {"error": f"执行 EXPLAIN 或处理结果时出错: {e}"}
    finally:
        if conn:
            conn.close()
                ...
                if len(add_index_fields) == 0:
                    ...
                    try:
                        ...
                        if not index_result:
                            suggestion_text = ""
                            if row['key'] is None:
                                suggestion_text += f"\033[93m建议添加索引：ALTER TABLE {table_name} ADD INDEX idx_{index_name}({index_columns});\033[0m\n"
                    ...
                    try:
                        ...
                        if index_result_list is None:
                            suggestion_text = ""
                            if row['key'] is None:
                                suggestion_text += f"\033[93m建议添加索引：ALTER TABLE {table_name} ADD INDEX idx_{merged_name}({merged_columns});\033[0m\n"
                            elif row['key'] is not None and row['rows'] >= 1:
                                suggestion_text += f"\033[93m建议添加索引：ALTER TABLE {table_name} ADD INDEX idx_{merged_name}({merged_columns});\033[0m\n"
                    ...
                    if len(add_index_fields) == 0:
                        ...
                        try:
                            ...
                            if not index_result:
                                suggestion_text = ""
                                if row['key'] is None:
                                    suggestion_text += f"\033[93m建议添加索引：ALTER TABLE {table_real_name} ADD INDEX idx_{index_name}({index_columns});\033[0m\n"
                                elif row['key'] is not None and row['rows'] >= 1:
                                    suggestion_text += f"\033[93m建议添加索引：ALTER TABLE {table_real_name} ADD INDEX idx_{index_name}({index_columns});\033[0m\n"
                        ...
                        try:
                            ...
                            if index_result_list is None:
                                suggestion_text = ""
                                if row['key'] is None:
                                    suggestion_text += f"\033[93m建议添加索引：ALTER TABLE {table_real_name} ADD INDEX idx_{merged_name}({merged_columns});\033[0m\n"
                                elif row['key'] is not None and row['rows'] >= 1:
                                    suggestion_text += f"\033[93m建议添加索引：ALTER TABLE {table_real_name} ADD INDEX idx_{merged_name}({merged_columns});\033[0m\n"
    ...
    try:
        ...
        #ai_suggestions = ai_suggestions.replace("If you want to run the SQL query, connect to a database first. See here: https://vanna.ai/docs/databases.html", "").strip()
        #ai_suggestions = ai_suggestions.replace("-------------------------------------------------------", "").strip()
    except Exception as e:
